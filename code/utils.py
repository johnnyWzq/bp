...
def save_model(model, feature_name, pkl_dir, depth=None):
    ...
    joblib.dump(model, pkl_dir+model_name+'.pkl')
    ...
    if model_name == 'DecisionTreeRegressor':
        tree.export_graphviz(model, pkl_dir+'dt_depth%s.dot'%str(depth),
                             feature_names=feature_name, max_depth=3)
    elif model_name == 'GradientBoostingRegressor':
        tree.export_graphviz(model.estimators_.ravel()[0], pkl_dir+'gbdt.dot',
                             feature_names=feature_name)
    elif model_name == 'RandomForestRegressor':
        tree.export_graphviz(model.estimators_[0], pkl_dir+'rf_depth%s.dot'%str(depth),
                             feature_names=feature_name, max_depth=3)